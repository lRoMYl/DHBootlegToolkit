...
extension JSONTreeView {
    public init(
        ...
        manuallyCollapsed: Binding<Set<String>> = .constant([]),
        onValueChange: @escaping ([String], Any) -> Void,
        onAddField: (([String]) -> Void)? = nil,
        onDeleteField: (([String]) -> Void)? = nil
    ) {
        self.json = json
        self.path = path
        ...
        self._manuallyCollapsed = manuallyCollapsed
        self.onValueChange = onValueChange
        self.onAddField = onAddField
        self.onDeleteField = onDeleteField
    }

    // Simpler initializer without manual collapse tracking
    public init(
        json: [String: Any],
        path: [String],
        currentMatchPath: [String]? = nil,
